...
meaninit.rsim<-rsim
for (i in 1:nrow(samples))
meaninit.rsim[i,T]<-colSums(rsim[harvest==0 & type==type[i], T])/4
mean(rsim[type=="KL"&harvest==0,T])-meaninit.rsim[23,T]
difinit.rsim<-rsim-meaninit.rsim

meaninit.rcTIC<-rcTIC
for (i in 1:nrow(samples))
meaninit.rcTIC[i,T]<-colSums(rcTIC[harvest==0 & type==type[i], T])/4
mean(rcTIC[type=="KL"&harvest==0,T])-meaninit.rcTIC[23,T]
difinit.rcTIC<-rcTIC-meaninit.rcTIC

qinit.rsim<-rsim[harvest!=0]/(meaninit.rsim[harvest!=0]*samples$massloss[harvest!=0])



meanh2.rsim<-rsim
for (i in 1:nrow(samples))
meanh2.rsim[i,T]<-colSums(rsim[harvest==2 & type==type[i], T])/length(rsim[harvest==2 & type==type[i], 1])
difh2.rsim<-rsim-meanh2.rsim
qh2.rsim<-rsim/meanh2.rsim

f<-peaks$class=="f.f"|peaks$class=="c.f"|peaks$class=="cp.f"
cp<-peaks$class=="f.cp"|peaks$class=="c.cp"|peaks$class=="cp.cp"
# 
# for (i in 1:peaknr)
# 	 cTIC[,i]<-sim[,i] * peaks$tic[i]
# 
