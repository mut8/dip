...
timeseries(orig_cTIC$L/(orig_cTIC$L+orig_cTIC$C), days, type,
           type="o", pch=pch, pt.bg=colscale, endsig=T, ylab="Lignin/(Lignin+Carbohydrates)", xlab="Litter incubation (days)", lwd=2)
axis(4, labels=F)
axis(3, labels=F)
...
timeseries(orig_cTIC$L/(orig_cTIC$L+orig_cTIC$C), days, type,
           ...
           massloss=samples$cons_acc_resp_litC, masslossSE=samples$cons_acc_resp_litC_se,
           yaxt="n", xlab="carbon loss (g CO2-C g-1 litter-C)", lwd=2, legend="topleft")
abline(h=100, lty=2)
axis(2, labels=F)
axis(3, labels=F)
axis(4)
...
par(mar=c(2, 0, 0.6,5))
barpr2(-1*closs.corr.orig_cTIC$C/initials.orig_cTIC$C, samples$cons_acc_resp_litC, days==475, days==181, ylim=c(0,2), yaxt="n", main="")
axis(4)
axis(2, labels=F)
                                        #mtext("Carbohydrates degraded (% of initial pool)\n days 0 - 181", side=4, padj=2)
                                        #barpr(closs.corr.orig_cTIC$L, closs.corr.orig_cTIC$C, days==181, days==0, ylim=c(-1,3))
...
formatC(tapply(alldata$DOC[cond]/10, alldata$Litter[cond], mean), digits=2, format="f") ,
...
paste( "(", formatC(tapply(alldata$DOC[cond]/10, alldata$Litter[cond], stderr), digits=2, format="f") , ")", sep=""),
...
stoech.init<-
cbind(stoech.init.mean[,1], stoech.init.se[,1], stoech.init.mean[,2], stoech.init.se[,2], stoech.init.mean[,3], stoech.init.se[,3], stoech.init.mean[,4], stoech.init.se[,4])

rownames(stoech.init) <- c("C (% d.w.)", "C extr (% d.w.)", "N (% d.w.)", "P (% d.w.)", "C:N (w/w)", "C:P (w/w)", "N:P (w/w)", "K (mg g-1)", "Ca (mg g-1)","Mg (mg g-1)", "Fe (ppm)", "Mn (ppm)","Zn (ppm)")
colnames(stoech.init) <- c(typlev[1], "(SE)", typlev[2], "(SE)",typlev[3], "(SE)", typlev[4], "(SE)")
...
<<init_table, results=tex, echo=F>>=
  print(
        xtable(stoech.init, caption="Litter stoichiometry and mineral elemental contents measured after 14 days incubation. Standard errors are stated in brackets (n=5). C extr stands for extractable carbon.",
               label="initstoech", align="rrlrlrlrl")
...
<<chemcorrtable, results=tex, echo=F, fig=F>>=
chem.corr.matrix<-corr.ab(data.frame(
      alldata[alldata$days==181, c("Mass.loss" , "Respiration", "Cellulase", "Peroxydase", "Phenoloxidase", "cell2phen", "cell2per","Gross.protein.depolymerization" , "Gross.amino.acid.immobilization","Gross.N.mineralization", "Gross.NH4.immobilization","Gross.nitrification", "Gross.NO3.immobilization" ,"Gross.P.mineralization","Gross.P.immobilization","Glucan.Gross.Depolymerisation", "Glucose.Gross.Consumption", "NH4.concentration","NO3.concentration","PO4.concentration","C_lit", "N_lit","P_lit","C.N_lit","C.P_lit","N.P_lit", "K.litter" ,"CA.litter", "MG.litter","FE.litter"  ,"MN.litter","ZN.litter","C_mic" ,"N_.mic","P_mic","C.N_mic","C.Pmic", "N.Pmic","DOC","CN_inbal", "CP_inbal","NP_inbal")]),orig_cTIC[days==181, c("L", "C", "N", "Ph")], tex=T)
                                  
#rownames cor corraltion table
rownames(chem.corr.matrix) <- c("Massloss", "Respiration", "Cellulase activity", "Phenoloxidase activity", "Peroxidase activity", "Phenoloxidase/Cellulase", "Peroxidase/Cellulase", "Protein depolymerization", "Amino acid immobilization", "N mineralization", "NH4 immobilization", "Nitrification", "NO3 immobilization", "P mineralization", "P immobilization", "Glucan depolymerization", "Glucose consumption", "NH4", "NO3", "PO4", "C litter", "N litter", "P litter", "C:N litter", "C:P litter", "N:P litter", "K litter", "Ca litter", "Mg litter", "Fe litter", "Mn litter", "Zn litter", "Cmic", "Nmic", "Pmic", "C:N mic", "C:P mic", "N:P mic", "extractable C", "C:N imbalance", "C:P imbalance", "N:P imbalance")

colnames(chem.corr.matrix)<-c("Lignin(pyr)", "Carbohydrates(pyr)", "Protein(pyr)", "Phenol(pyr)")
...
tab<-xtable(chem.corr.matrix, caption="caption", label="corrtable", align=rep("c",5) )
