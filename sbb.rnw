...
Litter microbial biomass is homeostatic during the first 6 month (no or marginally negative correlation between microbial stoichiometry and litter stoichiometry) \citep{Mooshammer2011}, but not after 15 month, when all three ratios show correlations (R 0.53 - 0.64, all p \textless 0.002, H\textsubscript{C:N}=\Sexpr{round(H.cn.h4, digits=2)}, H\textsubscript{C:P}=\Sexpr{round(H.cp.h4, digits=2)}, H\textsubscript{N:P}=\Sexpr{round(H.np.h4, digits=2)}). Microbial C:N ratios are tighly constrained after 3 (1:14.5 - 1:18.2) and 6 month (1:6.9 - 1:9.0), but significantly different between the two time points. C:P and N:P ratios are less constrained, with the highest variance between litter from different sites after 3 month incubation (Fig. \ref{fig:mb}).
...
#barp(orig_cTIC$Ph, days==181, days==97)
axis(4, labels=F)
#abline(h=1, col="black", lty=about:startpage2)
...
<<morecorr_prep, results=hide, echo=F>>=
corr1<-alldata[,c(10,37, 71,11,14,13,12,15:16,20,22,24, 38, 56, 39:43, 53:55,  75:77)]

for (i in 1:ncol(corr1)) {

if (i==1) {
corr1.h2.mean<-tapply(corr1[alldata$days==97,i],alldata$Litter[alldata$days==97], mean) } else {
corr1.h2.mean<-cbind(corr1.h2.mean,
tapply(corr1[alldata$days==97,i],alldata$Litter[alldata$days==97], mean)
)}

if (i==1) {
corr1.h3.mean<-tapply(corr1[alldata$days==181,i],alldata$Litter[alldata$days==181], mean) } else {
corr1.h3.mean<-cbind(corr1.h3.mean,
tapply(corr1[alldata$days==181,i],alldata$Litter[alldata$days==181], mean)
)}


if (i==1) {
corr1.h4.mean<-tapply(corr1[alldata$days==475,i],alldata$Litter[alldata$days==475], mean) } else {
corr1.h4.mean<-cbind(corr1.h4.mean,
tapply(corr1[alldata$days==475,i],alldata$Litter[alldata$days==475], mean)
)}

if (i==1) {
corr1.h34.mean<-rowMeans(cbind(corr1.h3.mean+corr1.h4.mean))} else {
corr1.h33.mean<-cbind(corr1.h34.mean,
rowMeans(cbind(corr1.h3.mean[,i]+corr1.h4.mean[,i]))
)}

if (i==1) {
corr1.h2.se<-tapply(corr1[alldata$days==97,i],alldata$Litter[alldata$days==97], stderr) } else {
corr1.h2.se<-cbind(corr1.h2.se,
tapply(corr1[alldata$days==97,i],alldata$Litter[alldata$days==97], stderr)
)}

if (i==1) {
corr1.h3.se<-tapply(corr1[alldata$days==181,i],alldata$Litter[alldata$days==181], stderr) } else {
corr1.h3.se<-cbind(corr1.h3.se,
tapply(corr1[alldata$days==181,i],alldata$Litter[alldata$days==181], stderr)
)}

if (i==1) {
corr1.h4.se<-tapply(corr1[alldata$days==475,i],alldata$Litter[alldata$days==475], stderr) } else {
corr1.h4.se<-cbind(corr1.h4.se,
tapply(corr1[alldata$days==475,i],alldata$Litter[alldata$days==475], stderr)
)}
...
for (i in 1:ncol(corr1)) {
if (i==1) {
corr1.h23.mean<-rowMeans(cbind(corr1.h2.mean[,i], corr1.h3.mean[,i]))} else {corr1.h23.mean<-cbind(corr1.h23.mean, rowMeans(cbind(corr1.h2.mean[,i],corr1.h3.mean[,i]))
)}

if (i==1) {
corr1.h34.mean<-rowMeans(cbind(corr1.h3.mean[,i],corr1.h4.mean[,i]))} else {
corr1.h34.mean<-cbind(corr1.h34.mean,
rowMeans(cbind(corr1.h3.mean[,i],corr1.h4.mean[,i]))
)}
}
...
if (i==1) {
hmw.proc.h3.means<-tapply(hmw.proc[,i],type[days==181], mean) } else {
hmw.proc.h3.means<-cbind(hmw.proc.h3.means, tapply(hmw.proc[,i],type[days==181], mean)
)}

if (i==1) {
hmw.proc.h4.means<-tapply(hmw.proc2[,i],type[days==475], mean) } else {
hmw.proc.h4.means<-cbind(hmw.proc.h4.means, tapply(hmw.proc2[,i],type[days==475], mean)
)}

if (i==1) {
hmw.proc.h3.se<-tapply(hmw.proc[,i],type[days==181], stderr) } else {
hmw.proc.h3.se<-cbind(hmw.proc.h3.se, tapply(hmw.proc[,i],type[days==181], stderr)
)}

if (i==1) {
hmw.proc.h4.se<-tapply(hmw.proc2[,i],type[days==475], stderr) } else {
hmw.proc.h4.se<-cbind(hmw.proc.h4.se, tapply(hmw.proc2[,i],type[days==475], stderr)
)}

}

names<-c("Massloss", "Actual respiration", "Accumulated Respiration", "Cellulase activity", "Protease activity","Phosphatase activity","Chitinase activity", "Phenoloxidase activity", "Peroxidase activity", #"Phenoloxidase/Cellulase", "Peroxidase/Cellulase", "Protein depolymerization", "Amino acid immobilization", 
"N mineralization", #"NH4 immobilization", 
"Nitrification", #"NO3 immobilization",
"P mineralization", #"P immobilization", "Glucan depolymerization", "Glucose consumption", "NH4", "NO3", "PO4", 
"C litter", "extractable C", "N litter", "P litter", "C:N litter", "C:P litter", "N:P litter",
...
colnames(corr1.h2.mean)<-names
colnames(corr1.h3.mean)<-names
colnames(corr1.h4.mean)<-names
colnames(corr1.h23.mean)<-names
colnames(corr1.h34.mean)<-names
colnames(corr1.h2.se)<-names
colnames(corr1.h3.se)<-names
colnames(corr1.h4.se)<-names

colnames(hmw.proc.h3.means)<-colnames(hmw.proc)
colnames(hmw.proc.h4.means)<-colnames(hmw.proc)
colnames(hmw.proc.h3.se)<-colnames(hmw.proc)
colnames(hmw.proc.h4.se)<-colnames(hmw.proc)
@

\newpage
<<morecorr_pyrh3_corh2, results=tex, echo=F>>=
tab<-xtable(corr.ab(corr1.h2.mean[,is.na(colSums(corr1.h2.mean))==F],hmw.proc.h3.means, tex=T), caption="H3 vs. H2", label="morecorr1", align=rep("c",11) )
...
\newpage
<<morecorr_pyrh3_corh3, results=tex, echo=F>>=
tab<-xtable(corr.ab(corr1.h3.mean[,is.na(colSums(corr1.h3.mean))==F],hmw.proc.h3.means, tex=T), caption="H3 vs. H3", label="morecorr2", align=rep("c",11) )

print(tab, size="tiny", sanitize.text.function = function(x){x}, table.placement = "h!", caption.placement = "top")
@

\newpage
<<morecorr_pyrh3_corh23, results=tex, echo=F>>=
tab<-xtable(corr.ab(corr1.h23.mean[,is.na(colSums(corr1.h23.mean))==F],hmw.proc.h3.means, tex=T), caption="H3 vs. mean (H2+H3)", label="morecorr3", align=rep("c",11) )

print(tab, size="tiny", sanitize.text.function = function(x){x}, table.placement = "h!", caption.placement = "top")
@

\newpage
<<morecorr_pyrh4_corh3, results=tex, echo=F>>=
tab<-xtable(corr.ab(corr1.h3.mean[,is.na(colSums(corr1.h3.mean))==F],hmw.proc.h4.means, tex=T), caption="H4 vs. H3", label="morecorr4", align=rep("c",11) )

print(tab, size="tiny", sanitize.text.function = function(x){x}, table.placement = "h!", caption.placement = "top")
@

\newpage
<<morecorr_pyrh4_corh4, results=tex, echo=F>>=
tab<-xtable(corr.ab(corr1.h4.mean[,is.na(colSums(corr1.h4.mean))==F],hmw.proc.h4.means, tex=T), caption="H4 vs. H4", label="morecorr1", align=rep("c",11) )

print(tab, size="tiny", sanitize.text.function = function(x){x}, table.placement = "h!", caption.placement = "top")
@

\newpage
<<morecorr_pyrh4_corh34, results=tex, echo=F>>=
tab<-xtable(corr.ab(corr1.h34.mean[,is.na(colSums(corr1.h34.mean))==F],hmw.proc.h4.means, tex=T), caption="H4 vs. mean (H3 + H4)", label="morecorr1", align=rep("c",11) )
