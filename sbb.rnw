...
<<init_stoech_table, results=tex, echo=F>>=
  print(xtable(stoech.init, caption="Litter stoichiometry measured after 14 days incubation. Error bars indicate standard errors (n=5).",
...
%  timeseries(alldata$f2b_qpcr[cond], alldata$days[cond], alldata$Litter[cond], type="o",pch=pch, col=colscale, log="y", ylab=”Fungi to bacteria ratio )
% @
% \end{center}
...
\subsection{Lignin and carbohydrate content}
...
When measured by pyr-GC/MS, lignin derrived compounds make up between 29 and 31 \%TIC in the initial litter, with an increase of up to 3 \%TIC over the first 3 month. The increase is highest in SW and lowest in AK, with the other litter type at intermediate levels. Differences are significant between SW and AK as well as between initial and 6 month harvests for SW.
...
\vspace*{2mm}
% \begin{center}
<<lci, fig=T, echo=F, results=hide, height=7, width=3.5>>=
par(oma=c(4,4,4,4), mfrow=c(1,2), mar=c(.1,.1,.1,.1))
...
massloss=samples$massloss, masslossSE= samples$masslossSE, yaxt="n")
abline(h=100)
axis(2, labels=F)
axis(4)
@
