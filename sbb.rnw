...
\fntext[boku]{Current adress: Institute for Soil Science, University of Natural Resources and Life Sciences, Peter Jordan-Straße 82, A-1190, Vienna, Austria}
...
During the first 6 month fundamental differences in lignin degrading activities were found between sites. Lignin discrimination in litter decomposition ranges between only marginal amounts of lignin being decomposed and lignin decomposition at the same rate average litter, leading to different niveaus of lignin accumulation. Between 6 and 15 month, no lignin discrimination was found, but different lignin contents aquired earlier remined.

Neither nitrogen nor labile carbon availability could be identified as a control over lignin decomposition. However, amounts of lignin decomposed were correlated to the buildup of resource to decomposer C:N and C:P inbalances.
...
<<init_stoech_table, results=tex, echo=F>>=
  print(
     ...
     label="initstoech", allign="rrrrrrr"), size = "tiny",table.placement = "tb", caption.placement = "top"  )
@
...
Substrate to consumer stoichiometric inbalances \emph{X:Y$_{inbal}$} were calculated as
...
timeseries(alldata$DOC/1000, alldata$days, alldata$Litter, pch=pch, col=colscale,  xlab="Incubation (days)", ylab="mg C g-1 d.w.", main="Extractable carbon", lwd=2)
legend("bottomright", col=colscale, pch=pch, typlev, lty=1, lwd=2)
@
\end{center}
\caption{Extractable organic carbon. Error bars indicate standard errors (n=5).x}
\label{fig:doc}
\end{figure*}
...
cond <- is.na(data)!=T
timeseries(data[cond], alldata$days[cond], alldata$Litter[cond],
type="o", pch=pch, col=colscale, endsig=T, main="Mic. C:P ratio",
...
%colnames(alldata)
%f2b
%f2b<-read.csv("raw_data/f2b.csv", header=T)


%plot(f2b$f2b_prot_h1, tapply(hmw.proc$LCI, type[h3], mean), pch=pch, col=colscale[4],
%      xlim=c(min(f2b[,c("f2b_prot_h1", "f2b_prot_h2", "f2b_prot_h3")]),
%              max(f2b[,c("f2b_prot_h1", "f2b_prot_h2", "f2b_prot_h3")])
%      )
%    )
%plot(f2b$f2b_prot_h2, tapply(hmw.proc$LCI, type[h3], mean), pch=pch, col=colscale[3])
%points(f2b$f2b_prot_h3, tapply(hmw.proc$LCI, type[h3], mean), pch=pch, col=colscale[1])
%abline(lm(tapply(hmw.proc$LCI, type[h3], mean) ~ f2b$f2b_prot_h1), col=colscale[4])
%abline(lm(tapply(hmw.proc$LCI, type[h3], mean) ~ f2b$f2b_prot_h2), col=colscale[3])
%abline(lm(tapply(hmw.proc$LCI, type[h3], mean) ~ f2b$f2b_prot_h3), col=colscale[1])


%plot(f2b$f2b_trans_h1, tapply(hmw.proc$LCI, type[h3], mean), pch=pch, col=colscale[4],
%      xlim=c(min(f2b[,c("f2b_trans_h1", "f2b_trans_h2")]),
%              max(f2b[,c("f2b_trans_h1", "f2b_trans_h2")])
%      )
%    )
%points(f2b$f2b_trans_h2, tapply(hmw.proc$LCI, type[h3], mean), pch=pch, col=colscale[3])
%abline(lm(tapply(hmw.proc$LCI, type[h3], mean) ~ f2b$f2b_trans_h1), col=colscale[4])
%cor.test(f2b$f2b_trans_h1, tapply(hmw.proc$LCI, type[h3], mean))
%abline(lm(tapply(hmw.proc$LCI, type[h3], mean) ~ f2b$f2b_trans_h2), col=colscale[3])
%cor.test(f2b$f2b_trans_h2, tapply(hmw.proc$LCI, type[h3], mean))   
%     
%cond<-alldata$days==97
%plot(tapply(hmw.proc$LCI, type[h3], mean), tapply(alldata$f2b_qpcr[cond], aslldata$Litter[cond], mean), pch=pch     )
%
%cor.test(
%alldata$f2b_qpcr[alldata$days=="181"]
%, hmw.proc$L_C_dec, pch=pch.h3 )
%
%cond <- is.na(alldata$f2b_qpcr)==F
%timeseries(alldata$f2b_qpcr[cond], alldata$days[cond], alldata$Litter[cond], pch=pch, %col=colscale, lwd=2)
%@
 % \end{center}
...
%
%write.csv(rbind(
%      corr.ab(alldata[h2, c(18:28, 37)], alldata[h2,11:16]),
...
%\footnote{still needs some work...}We quantified a number of other recalcitrant or lipophilic substances. Phenoles (most dominant phenol and 2/3-methylphenol) account for up to 8\% TIC and strongly increase in their abundance over the first 6 month.
...
hmw.proc <-
 data.frame(
  ...
  #log(closs.corr.orig_cTIC$L[h3] - initials.orig_cTIC$L[h3]) / (closs.corr.orig_cTIC$C[h3] - initials.orig_cTIC$C[h3])
  )
alldata$cell2phen<-alldata$Phenoloxidase/alldata$Cellulase
alldata$cell2per<-alldata$Peroxydase/alldata$Cellulase

colnames(hmw.proc) <- c("LTIC", "CTIC", "LCI", "Ldec", "Cdec", "Lresp", "Cresp", "LCdec"
                        #, "log(L:Cdec)"
                        )
...
Appendix states correlations found between lignin degradation, litter chemistry, microbial community parameters and decomposition processes after 6 month incubation. Lignin accumulation was correlated to high turnover rates in decomposition processes and high enzymatic activities. 

\begin{figure*}[t]
\vspace*{2mm}
\begin{center}
\setkeys{Gin}{width=0.6\textwidth}
<<graphcorr, fig=T, echo=F, results=hide, width=7, height=7>>=
par(mfrow=c(2,2))
...
var3<-hmw.proc$LCI
var4<-hmw.proc$LCdec
pch.h3<-c(rep(1,5), rep(16,5), rep(2,5), rep(17,5))

var1lab<-"C:N resource / C:N microbial"
var2lab<-"C:P resource / C:P microbial"
var3lab<-"LCI change"
var4lab<-"Lignin decomposition /\n Carbohydrate decomposition"

corplot<-function(v1,v2,v1lab, v2lab, ...)
{
plot(v1, v2, pch=pch.h3, xlab=v1lab, ylab=v2lab, ...)
abline(lm(v2~v1))     
cor<-cor.test(v2,v1)
text(max(v1)*.8, max(v2)*.95, labels=paste("R =", formatC(cor$estimate, digits=3), siglev(cor$p.value)), cex=0.8)
}

par(mar=c(4,5,1,0))
corplot(var1, var3, "", var3lab)
par(mar=c(4,4,1,1))
corplot(var2, var3, "", "")
par(mar=c(5,5,0,0))
corplot(var1, var4, var1lab, var4lab)
par(mar=c(5,4,0,1))
corplot(var2, var4, var2lab, "")
...
[C use efficiency lowest where most lignin decomposed.]


was strictly correlated to most decomposition processes measured [enzymes, N-depoly, Glucose-depoly, ... ] after 6 month and [test!]correlated to respiration at earlier harvest \citep{Mooshammer2011}. Earlier analysis of decomposition processes in the same samples found controls of N content and litter C:N ratios over decomposition processes \citep{Mooshammer2011, Leitner2011}. [The system is N limited, at least after 6 month.] However, N content is similar in AK and OS, so N content as a single factor can not explain the differences observed.
...
We found substancial changes in decomposition processes after 6 month\footnote{due to to low number of harvests, this study can not pin down the exact timepoint...}: 

(1) Lignin decomposition is occurs on the same rate as bulk litter mineralisation in all litter types, no further accumulation of lignin.

(2) Extractable carbon contents start decreases in most litter types until 6 month incubation and increase in all site afterwards. While initial extractable carbon content seems to be an arbitraty litter treat, after 6 month, exctractable carbon content is correlated to litter N and to decomposition processes, i.e. is actively produced. 

The same applies for litter DOC content: Higher DOC quantity in SW and AK lead to different trends, in SW lignin was most accumulated in AK the least.
...
mighty.corr.matrix<-
corr.ab(
alldata[days==181&alldata$pyr.excl==F,c(10, 11,15:16,77:78,18:26, 28, 34:56, 67, 71, 73:75)]
,hmw.proc, tex=T
)

#rownames cor corraltion table
rownames(mighty.corr.matrix)<-  c("Massloss", "Cellulase activity", "Phenoloxidase activity", "Peroxidase activity", "Phenoloxidase/Cellulase", "Peroxidase/Cellulase", "Protein depolymerization", "Amino acid immobilization", "N mineralization", "NH4 immobilization", "Nitrification", "NO3 immobilization", "P mineralization", "P immobilization", "Glucan depolymerization", "Glucose consumption", "NH4", "NO3", "PO4","Respiration", "C litter", "N litter", "P litter", "C:N litter", "C:P litter", "N:P litter", "K litter", "Ca litter", "Mg litter", "Fe litter", "Mn litter", "Zn litter", "Cmic", "Nmic", "Pmic", "C:N mic", "C:P mic", "N:P mic", "extractable C", "Fungi/bacteria (plfa 2)", "Fungi/bacteria (qPCR)", "C:N inbalance", "C:P inbalance", "N:P inbalance")

colnames(mighty.corr.matrix)<-c("L acc", "Ch acc", "LCI diff", "L dec", "C dec", "L resp", "C resp", "L/C dec")
...
tab<-
xtable(
   mighty.corr.matrix, caption="Correlation (R) between Lignin and Carbohydrate degration with litter chemistry, microbial community and decomposition processes. Significant (p<0.05) correlations are printed bold. Data taken from \\cite{Mooshammer2011, Leitner2011, Frank2011}", label="corrtable", align=rep("r",9) ) 

print(tab, size="tiny", sanitize.text.function = function(x){x}, table.placement = "!", caption.placement = "top")

@

%<<corrtable2, results=tex, echo=F>>=
% print(xtable(mighty.corr.matrix[,c(1,11:25)]) , size="Tiny", sanitize.text.function = function(x){x})
%@
