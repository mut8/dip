...
In this study we analyze samples from climate-chamber incubated beech litter varying in N and P content with pyrolysis-GC/MS (pyr-GC/MS). The experiment was designed to study microbial decomposition, exclude decomposing fauna and keep climatic conditions constant. Extensive data on litter chemistry and and decomposition process rates are available for this samples from previously publications \citep{Mooshammer2011, Wanek2011, Leitner2011}.
...
stoech.init <-
 data.frame ( paste( formatC(tapply(alldata$C_lit[cond], alldata$Litter[cond], mean), digits=2, format="f") , " (", formatC(tapply(alldata$C_lit[cond], alldata$Litter[cond], stderr), digits=2, format="f") , ")", sep=""),
...
names(stoech.init) <- c("C (% d.w.)", "N (%d.w.)", "P (%d.w.)", "C:N (w/w)", "C:P(w/w)", "N:P(w/w)")
rownames(stoech.init) <- typlev
stoech.init
@
<<init_stoech_table, results=tex, echo=F>>=
 print(
 xtable(stoech.init, caption="Litter stoichiometry measured after 14 days incubation. Error bars indicate standard errors (n=5).",
 label="initstoech",table.placement = "tbp", caption.placement = "top" , size = "small" )
 )
@
...
A detailed description of our litter decomposition experiment was published in \cite{Wanek2011}. Briefly, beech litter was collected at at four different sites in Austria (Achenkirch (AK), Klausenleopoldsdorf(KL), Ossiach(OS), and Schottenwald(SW); refered to as litter types) in October 2008. Litter was cut to pieces of approximately 0.25cm², homogenized, sterilized twice by \textgamma -radiation (35 kGy, 7 days between irradiations). Litter was inoculated (1.5\% w/w) with a mixture of litter and soil to assure that all litter types share the same initial microbial community. From each type, four samples of litter were taken after inoculation and stored dried at room temperature. Samples of 60g litter (fresh weight) were incubated at 15 \textdegree C and 60\% water content in mesocosms for a duration between 2 weeks to 15 month. For each litter type 5 replicas were removed and analyzed after 14, 97, 181 and 375 days. 
...
<<mb, fig=T, echo=F, results=hide, width=12, height=7>>=

 par(mfrow=c(3,3), mar=c(2.1,4.1,0.1,0.1), tck=0.01)
 data <- alldata$C_mic
 cond <- is.na(data)!=T
 timeseries(data[cond], alldata$days[cond], alldata$Litter[cond],
 type="o", pch=pch, col=colscale,
 ylab="microbial carbon (mg C g-1 d.w.)", endsig=T, xaxt="n")
 axis(4, labels=F)
 axis(3, labels=F)
 axis(1, labels=F)
 data <- alldata$N_.mic
 ...
 ylab="microbial nitrogen (mg N g-1 d.w.)", xaxt="n")
 axis(4, labels=F)
 axis(3, labels=F)
 axis(1, labels=F)
 ...
 timeseries(data[cond], alldata$days[cond], alldata$Litter[cond],
 type="o", pch=pch, col=colscale,
 ylab="microbial phosphorus (mg P g-1 d.w.)", xaxt="n", endsig=T)
 axis(4, labels=F)
 axis(3, labels=F)
 axis(1, labels=F)
 ...
 timeseries(data[cond], alldata$days[cond], alldata$Litter[cond],
 type="o", pch=pch, col=colscale, endsig=T,
 ylab="microbial C:N ratio (w/w)", xaxt="n")
 axis(4, labels=F)
 axis(3, labels=F)
 axis(1, labels=F)
 ...
 cond <- is.na(data)!=T
 timeseries(data[cond], alldata$days[cond], alldata$Litter[cond],
 type="o", pch=pch, col=colscale, endsig=T, ylab="microbial C:Pg ratio (w/w)", xaxt="n")
 axis(4, labels=F)
 axis(3, labels=F)
 axis(1, labels=F)
 ...
 cond <- is.na(data)!=T
 timeseries(data[cond], alldata$days[cond], alldata$Litter[cond],
 type="o", pch=pch, col=colscale, endsig=T, ylab="microbial N:P ratio (w/w)", xaxt="n")
 axis(4, labels=F)
 axis(3, labels=F)
 axis(1, labels=F)
 ...
 timeseries(data[cond], alldata$days[cond], alldata$Litter[cond],
 type="o", pch=pch, col=colscale, endsig=T,
 xlab="Litter incubation (days)", ylab="C:N inbalance")
 axis(4, labels=F)
 axis(3, labels=F)
 ...
 timeseries(data[cond], alldata$days[cond], alldata$Litter[cond],
 type="o", pch=pch, col=colscale, endsig=T,
 xlab="Litter incubation (days)", ylab="C:P inbalance")
 axis(4, labels=F)
 axis(3, labels=F)
 ...
 timeseries(data[cond], alldata$days[cond], alldata$Litter[cond],
 type="o", pch=pch, col=colscale, endsig=T,
 xlab="litter incubation (days)", ylab="N:P inbalance")
 axis(4, labels=F)
 axis(3, labels=F)
@
...
<<enz, fig=T, echo=F, results=hide>>=
par(mfrow=c(2,2), mar=c(2.5,5,2,0.5))
...
<<pca2, fig=T, echo=F, results=hide>>=
 ord.plot(rda((rsim-initials.rsim)/initials.rsim, scale=T), days, type,scale=T, col=colscale, pch=pch, spe.labels=peaks$origin, spe.mult=3, sep1.unit="days")
@
 \end{center}
 \caption{TEXT}
 \label{pca2}
...
%@
 ...
 Lignin ratios determined by ADF extraction show initial lignin contents between 15 and 21 \% d.w.. After 3 month, lignin contents rise to levels above 30\% d.w. in two sites, an increase that can not be explained by litter mass loss.
...
\begin{center}
\setkeys{Gin}{width=\textwidth}
<<degrdiff, fig=T, results=hide, echo=false, width=14>>=
par(mfrow=c(2,5))
 #barp(orig_cTIC$L, days==97, days==0)
 #barp(orig_cTIC$Ph, days==97, days==0)
 #barp(orig_cTIC$C, days==97, days==0)
...
par(mar=c(0.2, 5, 2,0))
barp(-1*closs.corr.orig_cTIC$L*100/initials.orig_cTIC$L, days==181, days==0, ylim=c(0,25), ylab="% of initial pool degraded \n days 0 - 181", main="Lignin")
#barp(orig_cTIC$Ph, days==181, days==97)
axis(4, labels=F)

par(mar=c(0.2, 0, 2,5))
barp(-1*closs.corr.orig_cTIC$C*100/initials.orig_cTIC$C, days==181, days==0, ylim=c(0,25), yaxt="n", main="Carbohydrates")
axis(4)
axis(2, labels=F)
#mtext("Carbohydrates degraded (% of initial pool)\n days 0 - 181", side=4, padj=2)
#barpr(closs.corr.orig_cTIC$L, closs.corr.orig_cTIC$C, days==181, days==0, ylim=c(-1,3))
...
par(mar=c(0.2, 0, 2,5))
barpr2(-1*closs.corr.orig_cTIC$C/initials.orig_cTIC$C, samples$cons_acc_resp_litC, days==181, days==0, ylim=c(0,2), yaxt="n", main="Carbohydrates")
axis(4)
axis(2, labels=F)
#mtext("Carbohydrates degraded (% of initial pool)\n days 0 - 181", side=4, padj=2)
#barpr(closs.corr.orig_cTIC$L, closs.corr.orig_cTIC$C, days==181, days==0, ylim=c(-1,3))
...
par(mar=c(0.2, 0, 2,5))
barpr(closs.corr.orig_cTIC$L/initials.orig_cTIC$L, closs.corr.orig_cTIC$C/initials.orig_cTIC$C, days==181, days==0, ylim=c(0,2), yaxt="n", main="Lignin/Carbohydrates"
      #, log="y"
      )
axis(4)
axis(2, labels=F)
...
The data presented demonstrates lignin decomposition during the early beech litter decomposition in a laboratory mesocosm environment. Lignin decomposition rates vary between marginal amounts and bulk carbon mineralization rates during the first 6 month of incubation and uniformly reach average carbon decomposition rates between 6 an 15 month. Therefore, we can confirm the suggestion that early lignin decomposition rates are by far underestimated, recently presented by \cite{Klotzbucher2011}, using a complementary anlytical approach. Unlike their results, we found only a minimal decrease in lignin contents in one litter type during early decomposition. Also, lignin degradation rates were constant or increased during decomposition. 

Increases in AUR observed during early decomposition result not from lignin accumulation, but from other compounds co-isolated in this unspecific method. Absolute increases in AUR contents also were found by other studies (i.e. \cite{Berg1980}), but was usually neglected. \footnote{an extensive review on this point in the numerous publications using ADF lignin determination  in litter decomposition experiments is out of the range of the current work.} 
