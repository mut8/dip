...
Our results demonstrate that relevant amounts of lignin are degraded during the the first 6 month of litter decomposition, and thereby contradict traditional concepts of lignin decomposition. Early lignin decomposition rates depend on litter quality and ranges from non-significant amounts decomposed to degradation at bulk carbon mineralization rates (no discrimination against lignin). We can therefore confirm that early lignin decomposition rates are by far underestimated, as proposed by \cite{Klotzbucher2011}, based on a complementary analytic method. Contrasting their results, we found no significant decreases in lignin contents and constant or increasing lignin degradation rates during early decomposition. We found a change in controls over lignin discrimination after this initial period. While the preference of carbohydrate over lignin decomposition were controlled by litter chemistry over the first 6 month, all components of litter were degraded at similar rates thereafter.

The experimental approach chosen allows us to single out litter quality and it's influence on the development of the microbial decomposer community as the sole source of the differences in decomposition processes found, while excluding fauna, climate and the initial microbial community as controlling factors. By exploiting intra-specific differences in beech litter stoichiometry, we were able to minimize differences in the composition of initial lignin and carbohydrate contents while exploring the effect of different litter nutrient contents. Therefore, we can attribute different levels of carbohydrate-over-lignin preference encountered to the intrinsic qualities of different litter types used.

Among the controls of litter chemistry over late lignin decomposition proposed, we can exclude limiting lignin contents and limitation by the lack of an essential micro-nutrient. In the traditional model for chemical changes during litter decomposition \cite{Berg1982} suggest lignin is decomposed only if its high content in litter limits the decomposition of other substrates. We found differences in initial lignin contents below 10\%, and lignin contents of sites with high initial lignin decomposition rates were not higher than that of sites with low rates. 
...
Several experiments found lignin decomposition inhibited by N fertilization, and it was suggested that microbial degradation of recalcitrant carbon compounds is directed to access nitrogen, not carbon. This 'nitrogen mining hypothesis' was supported by experiments indicating that cellulose addition 

Litter lignin was shown to occlude significant amounts of nitrogen during it's polymerization \footnote{citation!}. According to the 'nitrogen mining hypothesis' \citep{Craine2007},  

is reported to be inhibited by the availability of labile carbon \citep{Klotzbucher2011}, redox-active micronutrients (mainly Mn) \footnote{Lit.!} or N fertilization .  as key controls for (late) lignin degradation.

Between 6 and 15 month lignin contents remained constant in all litter types. This indicates that lignin is not degraded slower than other litter compounds, but differences in lignin contents aquired during the first 6 month remain. Controls over lignin decomposition which are in effect during the first 6 month are no longer predominant between 6 and 15 month. 
cv
...
corr1<-alldata[alldata$days==181,c(10,37, 71,11,14,13,15:16,20,22,24, 38, 56, 39:43, 53:55,  75:77)]
corr2<-hmw.proc
mighty.corr.matrix<-corr.ab(corr1, corr2, tex=T)
#rownames cor corraltion table
rownames(mighty.corr.matrix) <- 
  c("Massloss", "Actual respiration", "Accumulated Respiration", "Cellulase activity", "Protease activity","Phosphatase activity", "Phenoloxidase activity", "Peroxidase activity", #"Phenoloxidase/Cellulase", "Peroxidase/Cellulase", "Protein depolymerization", "Amino acid immobilization", 
...
tab<-xtable(mighty.corr.matrix, caption="Correlation (R) between Lignin and Carbohydrate degration with litter chemistry, microbial community and decomposition processes. Significant (p<0.05) correlations are printed bold. Data taken from \\cite{Mooshammer2011, Leitner2011}", label="corrtable", align=rep("c",11) )
...
corr1<-alldata[alldata$days==475,c(10,37, 71,11,14,13,15:16,20,22,24, 38, 56, 39:43, 53:55,  75:77)]
corr2<-hmw.proc2
...
tab<-xtable(mighty.corr.matrix2, caption="-->Correlation (R) between Lignin and Carbohydrate degration with litter chemistry, microbial community and decomposition processes. Significant (p<0.05) correlations are printed bold. Data taken from \\cite{Mooshammer2011, Leitner2011}<--", label="corrtable", align=rep("c",11) )
