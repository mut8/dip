...
Lignin is generally considered as the most recalcitrant component of plant litter, accumulated during early decomposition and degraded only during later decomposition stages when decomposition is limited by lignin concentration. A recent study based on the more specific (CuO-oxidation) to determine lignin contents reports challenges this concept and reports highest lignin decomposition rates during early litter decomposition. Until now, no further studies explore early lignin decomposition was published, and its potential controls remain unknown.

In this study we follow early litter decomposition with analytical pyrolysis in a climate-chamber decomposition experiment. We focus on resource control over early lignin decomposition and microbial carbon substrate preferences. We incubate beech litter collected at different sites after assuring an identical initial microbial community at constant climatic conditions to identify the control of litter chemistry on the developing microbial community and its decomposition activity.
...
<<init_stoech_table, results=tex, echo=F>>=
 print(
 xtable(stoech.init, caption="Litter stoichiometry measured after 14 days incubation. Error bars indicate standard errors (n=5). \footnote{insert more data (DOC!), table font smaller}",
...
<<lci, fig=T, echo=F, results=hide, height=3.5>>=
...
@
\end{center}
\caption{Lignin to carbohydrate index. Errorbars indicate standard errors (n=4-5)}
\label{fig:lci}
\end{figure*}
...
@
\end{center}
\caption{Lignin and Carbohydrated degraded (carbon loss corrected)}
\label{fig:degr}
\end{figure*}
...
Unlike data presented by \cite{Klotzbucher2011} we find only a minimal decrease in the lignin contents of one litter type and no decrease in lignin degradation rates during decomposition. \footnote{check if this is true: lignin is degraded slower in AK!} On the other hand do find substancial amounts of litter degraded during the first 6 month.

Our exeriment demonstrates that differences in resource quality can lead to the development of different substrate preferences in the same initial microbial community, profoundly affecting litter carbon chemistry during early litter decomposition.

During the first 6 month of decomposition, the absolute amount of lignin and the ratio in which lignin and carbohydrates are degraded is strongly influenced by litter quality, ragning from marginal lignin degradation to lignin being degraded at same rate as bulk litter. This lead to different lignin contents in litter, which were preserved after 15 month incubation.
