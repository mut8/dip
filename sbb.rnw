...
Accounting for carbon loss, we estimate \% lignin and cellolose degraded during decomposition:

\begin{equation}
\%_{loss} = 100\cdot\frac{TIC_{init}-TIC_{act}}{TIC_{init}}\cdot\frac{\left ( 1-R_{acc}\right ) }{C_{init}}
\end{equation}

Where \emph{$TIC_{init}$} and \emph{$TIC_{act}$} stand for initial and actual \%TIC area of lignin or cellulose pyrolysis products, \emph{$C_{init}$} for the initial amount of C and \emph{$R_{acc}$} for the accumulated CO$_2$-C respired by a mesocosm.

We use the terms "accumulation" and depletion to refer to changes in litter composition and "degradation" to refer to the percent of initial lignin and carbohydrates decomposed.
% check pyridol peak!!
...
<<graphcorr, fig=T, echo=F, results=hide, width=10.5, height=7>>=
par(mfrow=c(2,3))
...
par(mar=c(4,5,1,0))
corplot(var1, var4, "", var4lab)
par(mar=c(4,4,1,1))
corplot(var2, var4, "", "")
par(mar=c(4,4,1,1))
corplot(var3, var4, "", "")
par(mar=c(5,5,0,0))
corplot(var1, var5, var1lab, var5lab)
par(mar=c(5,4,0,1))
corplot(var2, var5, var2lab, "")
par(mar=c(5,4,0,1))
corplot(var3, var5, var3lab, "")
...
mighty.corr.matrix<-
 corr.ab(alldata[alldata$days==181,c(10, 37, 11,15:16,77:78,18:26, 28, 34:36, 38:56, 73:75#, 79:84
)],hmw.proc, tex=T
)
#rownames cor corraltion table
rownames(mighty.corr.matrix) <- c("Massloss", "Respiration", "Cellulase activity", "Phenoloxidase activity", "Peroxidase activity", "Phenoloxidase/Cellulase", "Peroxidase/Cellulase", "Protein depolymerization", "Amino acid immobilization", "N mineralization", "NH4 immobilization", "Nitrification", "NO3 immobilization", "P mineralization", "P immobilization", "Glucan depolymerization", "Glucose consumption", "NH4", "NO3", "PO4", "C litter", "N litter", "P litter", "C:N litter", "C:P litter", "N:P litter", "K litter", "Ca litter", "Mg litter", "Fe litter", "Mn litter", "Zn litter", "Cmic", "Nmic", "Pmic", "C:N mic", "C:P mic", "N:P mic", "extractable C", "C:N inbalance", "C:P inbalance", "N:P inbalance"#, "CN extr", "CP extr", "NP extr", "CN immo", "CP immo", "NP immo"
)
...
tab<-xtable(   mighty.corr.matrix, caption="Correlation (R) between Lignin and Carbohydrate degration with litter chemistry, microbial community and decomposition processes. Significant (p<0.05) correlations are printed bold. Data taken from \\cite{Mooshammer2011, Leitner2011, Frank2011}", label="corrtable", align=rep("r",9) ) 
