...
timeseries(orig_cTIC$L/(orig_cTIC$L+orig_cTIC$C), days, type,
           type="o", pch=pch, pt.bg=colscale, normalize=1,
...
par(mar=c(2, 5, 0.6,0))
barp(-1*closs.corr.orig_cTIC$L*100/initials.orig_cTIC$L, days==475, days==181, ylim=c(0,20), ylab="% of initial pool degraded \n days 181 - 475", names.arg=typlev, main="Lignin")
                                        #barp(orig_cTIC$Ph, days==181, days==97)
axis(4, labels=F)

par(mar=c(2, 0, 0.6,5))
barp(-1*closs.corr.orig_cTIC$C*100/initials.orig_cTIC$C, days==475, days==181, ylim=c(0,20), yaxt="n", names.arg=typlev, main="Carbohydrates")
axis(4)
axis(2, labels=F)
                                        #mtext("Carbohydrates degraded (% of initial pool)\n days 0 - 181", side=4, padj=2)
                                        #barpr(closs.corr.orig_cTIC$L, closs.corr.orig_cTIC$C, days==181, days==0, ylim=c(-1,3))
...
par(mar=c(2, 0, 0.6,5))
barpr2(-1*closs.corr.orig_cTIC$C/initials.orig_cTIC$C, samples$cons_acc_resp_litC, days==475, days==181, ylim=c(0,2), yaxt="n", main="Carbohydrates", names.arg=typlev)
axis(4)
axis(2, labels=F)
                                        #mtext("Carbohydrates degraded (% of initial pool)\n days 0 - 181", side=4, padj=2)
                                        #barpr(closs.corr.orig_cTIC$L, closs.corr.orig_cTIC$C, days==181, days==0, ylim=c(-1,3))
...
par(mar=c(2, 0, 0.6,5))
barpr(closs.corr.orig_cTIC$L/initials.orig_cTIC$L, closs.corr.orig_cTIC$C/initials.orig_cTIC$C, days==475, days==181, ylim=c(0,2), yaxt="n",names.arg=typlev, main="Lignin/Carbohydrates"
                                        #, log="y"
      )
axis(4)
axis(2, labels=F)
...
var1lab<-"C:N inbalance"
var2lab<-"C:P inbalance"
var3lab<-"N:P inbalance"
var4lab<-"Lignin accumulation (%TIC difference)"
var5lab<-"Carbohydrates accumulation (%TIC difference)"
var6lab<-"Lignin decomposition (% of initial pool)"
var7lab<-"Carbohydrates decomposition (% of initial pool)"
...
stoech.init <-data.frame (
                          ...
                          paste( "(", formatC(tapply(alldata$C_lit[cond], alldata$Litter[cond], stderr), digits=2, format="f") , ")", sep=""),
...
colnames(stoech.init) <- c("C", "(% d.w.)", "C extr","(% d.w.)", "N","(% d.w.)", "P", "(% d.w.)", "C:N","(w/w)", "C:P","(w/w)", "N:P", "(w/w)")
rownames(stoech.init) <- typlev

cond <- alldata$Harvest=="I"
minerals.init <-data.frame (
                            ...
                            formatC(tapply(alldata$MG[cond], alldata$Litter[cond], mean), digits=3, format="f") ,
                            paste(  " (", formatC(tapply(alldata$MG[cond], alldata$Litter[cond], stderr), digits=3, format="f"), ")", sep=""),
                            formatC(tapply(alldata$FE[cond], alldata$Litter[cond], mean), digits=0, format="f"),
                            paste("(", formatC(tapply(alldata$FE[cond], alldata$Litter[cond], stderr), digits=3, format="f"), ")", sep=""),
                            formatC( tapply(alldata$MG[cond], alldata$Litter[cond], mean), digits=3, format="f"),
                            paste( " (", formatC(tapply(alldata$MG[cond], alldata$Litter[cond], stderr), digits=3, format="f"), ")", sep=""),
                            formatC(tapply(alldata$ZN[cond], alldata$Litter[cond], mean) , digits=2, format="f"),
                            paste("(", formatC(tapply(alldata$ZN[cond], alldata$Litter[cond], stderr), digits=2, format="f"), ")", sep="")
                            )
colnames(minerals.init) <- c("K","mg g-1", "Ca","mg g-1","Mg","mg g-1", "Fe", "ppm", "Mn", "ppm","Zn", "ppm")
rownames(minerals.init) <- typlev
@

\newpage
<<init_stoech_table, results=tex, echo=F>>=
  print(
        xtable(stoech.init, caption="Litter stoichiometry measured after 14 days incubation. Standard errors are stated in brackets (n=5). C extr stands for extractable carbon.",
               label="initstoech", align="rrlrlrlrlrlrlrl")
        , size = "tiny",table.placement = "h!", caption.placement = "top"  )
@

\newpage
<<init_minerals_table, results=tex, echo=F>>=
  print(
        xtable(minerals.init, caption="Mineral element content measured 14 days after incubarion. Standard errors are stated in brackets (n=5). All contents are given per g dry weight.",
               label="initstoech", align="rrlrlrlrlrlrl")
        , size = "tiny",table.placement = "h!", caption.placement = "top"  )
@
...
<<corrtable, results=tex, echo=F, fig=F>>=
  mighty.corr.matrix<-
  corr.ab(
    alldata[alldata$days==181,c(10, 37, 11,15:16,77:78,18:26, 28, 34:36, 38:56, 73:75)],
