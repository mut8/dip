...
In this study we analyze samples from climate-chamber incubated beech litter varying in N and P content with pyrolysis-GC/MS (pyr-GC/MS). The experiment was designed to study the effect of resource stoichiometry on microbial decomposition, exclude decomposing fauna and keep climatic conditions constant. We address the following questions:
...
(3) Do high lignin degradation rates correspond to higher fungal activity?
...
\subsection*{Fungi:Bacteria ratio}
...
For the decomposition of litter lignin and carbohydrates, microbial decomposers rely on the production and excretion of hydrolytic and oxidative extracellular enzymes. While the absolute amounts, in which these enzymes where produced, was controlled by N availability, the ratio in which they were produced corresponded to differences in cellulose and lignin degradation (fig.\ref{fig:enz}). While the mode of this regulation is unknown, we find a corresponding shift in the microbial community composition. Fungi colonize litter faster than bacteria and therefore dominate early litter decomposition, but the bacteria/fungi ratio decrease over the entire incubation period. Talbot et. al. \cite{Talbot2011} suggested that lignin decomposition might be interpreted a k-strategy used by microbes to be able to colonize more lignin-rich and nutrient-poor substrates, while r-strategist grow fast based on labile carbon. Low nutrient availability might favor this strategy, as the high P- demands of a fast growing microbial community can not be met. Indeed we found that lignin degradation is higher in litter, where the C:P and N:P are highest, i.e. high P demand limits growth.\footnote{naja..}

While we found different levels of lignin degradation during the first 6 month, lignin contents remained constant in all litter types between 6 and 15 month. This indicates that lignin was decomposed at the same rate as other litter compounds, but differences in lignin contents acquired during the first 6 month remain in place. The controls which lead to differences in the extend of lignin discrimination over the first six month are no longer predominant between 6 and 15 month. Lignin loss in this decomposition stage was positively correlated to litter N content, as was litter mass loss and respiration. The higher fungi/bacteria ratio was higher than during initial decomposition, and differences between sites became  (especially after 15 months incubation). Fungi, which grew fast in N and P rich litter become out-competed by a bacteria-dominated lignin degrading community.

The change in decomposition dynamics corresponds to change in soluble carbon. While during the first 3 month, extractable carbon contents were not or to a lesser extend correlated to litter N, soluble carbon is strictly correlated to litter N and actual respiration after 6 and 15 month\footnote{stats}. Kltzb\"{ü}cher et. al. \cite{Klotzbucher2011} suggest a change in decomposition dynamics after 100 to 200 days of incubation, after which lignin decomposition rates decrease due to lack of labile carbon. They also report a correlation between respiration rates and extractable carbon after this change. The authors interpret this correlation as carbon limitation to respiration, and suggest that lignin decomposition is inhibited under such a limitation. We can confirm the correlation between extractable carbon and respiration after 181 days, but not the inhibition of lignin decomposition. Also, we found that both respiration and the production of soluble carbon are both controlled by litter N content. The process of degrading macromolecular compounds into soluble molecules is conducted by extracellular enzymes and is therefore N intensive that the mineralization of labile carbon, depolymerization is the point in the decomposition process where a N limitation is most likely to become effective. 
...
Our results further question the concept that lignin decomposition is inhibited until late decomposition. While traditional litter decomposition concepts locate lignin decomposition only during late decomposition, we find substancial amounts of lignin decomposed over the first six month. The extend to which lignin is decomposed was controlled by litter chemistry over the first 6 month. However, we did not find lignin decomposition rates controlled by litter quality between 6 and 15 month. Also, soluble carbon contents were not restrictive to lignin decomposition.

While carbohydrate decomposition was stimulated by high N contents, early lignin decomposition rates were highly correlated C:P ratio and resource:consumer imbalances. High lignin contents accumulated during this stages remained in place during later decomposition. Litter with high lignin decomposition rates had higher bacterial  abundance. 

For further studies, this raises the question, to which extend late decomposition is influenced by this early, stoichiometry-controlled accumulation of recalcitrant compounds. 
...
%\setkeys{Gin}{width=4in}
\setkeys{Gin}{width=\textwidth}
<<enz, fig=T, echo=F, results=hide, height=7, width=7>>=
par(mfrow=c(2,2), mar=c(2,5,2,0))
timeseries(alldata$Cellulase, alldata$days, alldata$Litter, type="o",
           pch=pch, pt.bg=colscale, log="y", endsig=T, main="Cellulase", lwd=2)
mtext("potential activity (µmol min-1 g-1 d.w.", side=2, las=0, padj=-4)

peroxy.tmp<-alldata$Peroxydase
peroxy.tmp[alldata$days==475]<-rep(c(0.008, 0.009, 0.01, 0.011,0.012), 4)

par(mar=c(2,3,2,2))
timeseries(peroxy.tmp,
           alldata$days,
           alldata$Litter, type="o", pch=pch,cex=2,
           pt.bg=colscale, log="y", endsig=T, main="Peroxydase", lwd=2)

par(mar=c(2.5,5,2.5,0))
timeseries(alldata$Phenoloxidase, alldata$days, alldata$Litter, type="o", pch=pch, pt.bg=colscale, log="y", endsig=T, main="Phenoloxydase", lwd=2)

par(mar=c(2.5,3,2.5,2))
timeseries(alldata$Phenoloxidase/alldata$Cellulase, alldata$days,alldata$Litter, type="o", pch=pch, pt.bg=colscale, log="y", endsig=T, main="Phenoloxysdase/Cellulase", lwd=2)
@
\end{center}
\caption{
...
timeseries(orig_cTIC$L/(orig_cTIC$L+orig_cTIC$C), days, type,
           type="o", pch=pch, pt.bg=colscale, endsig=T, ylab="Lignin/(Lignin+Carbohydrates)", xlab="Litter incubation (days)", lwd=2, cex=1.5)
axis(4, labels=F)
axis(3, labels=F)

par(mar=c(4.1,.1,2.1,4.1))
timeseries(orig_cTIC$L/(orig_cTIC$L+orig_cTIC$C), days, type,
           type="o", pch=pch, pt.bg=colscale, normalize=1, legsig=F, endsig=T,
           massloss=samples$cons_acc_resp_litC, masslossSE=samples$cons_acc_resp_litC_se, yaxt="n", xlab="carbon loss (g CO2-C g-1 litter-C)", lwd=2, legend="topleft", cex=1.5)
abline(h=100, lty=2)
axis(2, labels=F)
axis(3, labels=F)
axis(4)
...
par(mar=c(2, 0, 0.6,5))
barpr(closs.corr.orig_cTIC$L/initials.orig_cTIC$L, closs.corr.orig_cTIC$C/initials.orig_cTIC$C, days==181, days==0, ylim=c(0,2), yaxt="n",names.arg=typlev, main="Lignin/Carbohydrates"
#, log="y"
      )
axis(4)
axis(2, labels=F)
mtext("Ratio (% of initial pools degraded)\n0-181 days", side=4, padj=2, las=0, cex=.7)
abline(h=1, col="black", lty=2)

#par(mar=c(2, 5, 0.6,0))
#barp(-1*closs.corr.orig_cTIC$L*100/initials.orig_cTIC$L, days==475, days==181, ylim=c(0,25), ylab="% of initial pool degraded \n days 181 - 475", names.arg=typlev, main="")
#barp(orig_cTIC$Ph, days==181, days==97)
#axis(4, labels=F)

#par(mar=c(2, 0, 0.6,5))
#barp(-1*closs.corr.orig_cTIC$C*100/initials.orig_cTIC$C, days==475, days==181, #ylim=c(0,25), yaxt="n", names.arg=typlev, main="")
#axis(4)
#axis(2, labels=F)
#mtext("Carbohydrates degraded (% of initial pool)\n days 0 - 181", side=4, padj=2)
#barpr(closs.corr.orig_cTIC$L, closs.corr.orig_cTIC$C, days==181, days==0, ylim=c(-1,3))
...
par(mar=c(2, 0, 0.6,5))
barpr(closs.corr.orig_cTIC$L/initials.orig_cTIC$L, closs.corr.orig_cTIC$C/initials.orig_cTIC$C, days==475, days==181, ylim=c(0,2), yaxt="n",names.arg=typlev, main=""
#, log="y"
      )
axis(4)
axis(2, labels=F)
mtext("Ratio (% of initial pools degraded)\n days 181-475", side=4, padj=2, las=0, cex=.7)
abline(h=1, col="black", lty=2)
@
\end{center}
...
CUE<-alldata$C.N_mic/(alldata$C.N_mic+  alldata$conc_acc_resp_dw *100)
cond<-is.na(CUE)==F
